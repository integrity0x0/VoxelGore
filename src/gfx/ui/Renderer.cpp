...
Renderer::Renderer(const vkcore::Device& device, VkRenderPass renderPass,
    ...
    : device_(&device),
      ...
      pipelineLayout_(device,
                      std::to_array({&descriptorSetLayout_}),
                      {}),
      pipeline_(
          vkcore::GraphicsPipelineCreator(device)
              .AddShaderStage(core::kAssetsPrefix + "shaders/ui.vert.spv",
                              VK_SHADER_STAGE_VERTEX_BIT)
              .AddShaderStage(core::kAssetsPrefix + "shaders/ui.frag.spv",
  ...
  mapped_ = static_cast<Vertex*>(vertexBuffer_.memorySlice().map(0));
}
