...
vkcore::Pipeline BillboardPipelines::BuildPipeline(const vkcore::Device& device,
  ...
  std::string fragmentPath = (renderLayer != RenderLayer::Cutout)
                                 ? core::kAssetsPrefix + "shaders/particle.frag.spv"
                                 : core::kAssetsPrefix + "shaders/particle_cutout.frag.spv";

  return creator
      .AddShaderStage(core::kAssetsPrefix + "shaders/particle.vert.spv", VK_SHADER_STAGE_VERTEX_BIT)
      .AddShaderStage(fragmentPath, VK_SHADER_STAGE_FRAGMENT_BIT)
...
BillboardPipelines::BuildPipelines(const vkcore::Device& device,
                                   const vkcore::RenderPass& renderPass) {
  return {
      BuildPipeline(device, renderPass, RenderLayer::Solid),
      BuildPipeline(device, renderPass, RenderLayer::Cutout),
      BuildPipeline(device, renderPass, RenderLayer::Translucent),
  };
}
...
BillboardPipelines::BillboardPipelines(const vkcore::Device& device,
    ...
    : textureSetLayout_(BuildTextureSetLayout(device)),
      ...
      pipelines_(BuildPipelines(device, renderPass)) {}
}  // namespace gfx