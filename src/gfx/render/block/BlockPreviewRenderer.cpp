...
vkcore::Texture BlockPreviewRenderer::CreateDepthTexture(const vkcore::Device& device,
...
vkcore::RenderPass BlockPreviewRenderer::BuildRenderPass(const vkcore::Device& device) {
...
vkcore::DescriptorSetLayout BlockPreviewRenderer::BuildDescriptorSetLayout(
...
vkcore::DescriptorPool BlockPreviewRenderer::BuildDescriptorPool(const vkcore::Device& device) {
...
vkcore::PipelineLayout BlockPreviewRenderer::BuildPipelineLayout(const vkcore::Device& device) {
...
vkcore::Buffer BlockPreviewRenderer::BuildVertexBuffer(uint32_t blockId) {
  ...
  for (uint32_t face = 0; face < 6; face++) {
    ...
    for (uint32_t v = 0; v < 6; ++v) {
      ...
      const BlockPreviewVertex& base = kFaceVertices[idx];
...
vkcore::Pipeline BlockPreviewRenderer::BuildPipeline(const vkcore::Device& device,
                                                const ShaderCompiler& shaderCompiler) {
  vkcore::ShaderModule vert = CompileShaderModule(
      shaderCompiler, device, core::kShadersPrefix + "blockPreview.vert", shaderc_vertex_shader);
  vkcore::ShaderModule frag = CompileShaderModule(
      shaderCompiler, device, core::kShadersPrefix + "blockPreview.frag", shaderc_fragment_shader);
  ...
  return vkcore::GraphicsPipelineCreator(device)
      ...
      .AddVertexBinding(0, sizeof(gfx::BlockPreviewVertex), VK_VERTEX_INPUT_RATE_VERTEX)
