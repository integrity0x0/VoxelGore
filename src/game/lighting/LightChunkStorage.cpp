...
uint8_t LightChunkStorage::GetLight(const glm::ivec3& worldPos) const {
  glm::ivec3 chunkPos, localPos;
  SplitWorldPos(worldPos, chunkPos, localPos);

  const LightChunk* chunk = GetChunk(chunkPos);
  if (chunk == nullptr) {
    return 0;
  }
...
void LightChunkStorage::SetLight(const glm::ivec3& worldPos, uint8_t strength) {
  glm::ivec3 chunkPos, localPos;
  SplitWorldPos(worldPos, chunkPos, localPos);

  LightChunk& chunk = GetOrCreateChunk(chunkPos);
